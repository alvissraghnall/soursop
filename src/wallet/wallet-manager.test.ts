...
describe("WalletManager", () => {
  ...
  describe("generateWallet", () => {
    it("should generate a wallet with valid keys and mnemonic", async () => {
      ...
      const publicKeyBuffer = new Uint8Array(
        (await crypto.subtle.exportKey("spki", wallet.publicKey)).slice(12),
      );
  ...
  describe("importFromMnemonic", () => {
    ...
    it("should throw an error for invalid mnemonic", async () => {
      await expect(
        ...
        "Failed to import from mnemonic: Error: Invalid mnemonic phrase",
      );
    });
  });
  ...
  describe("importFromPrivateKey", () => {
    const compareWallets = async (
      imported: WalletInfo,
      generated: WalletInfo,
    ) => {
      const [importedPriv, importedPub] =
        await walletManager.exportKeyPair(imported);
      const [generatedPriv, generatedPub] =
        await walletManager.exportKeyPair(generated);
    ...
    it("should throw for malformed private key", async () => {
      await expect(
        walletManager.importFromPrivateKey("badKey"),
  ...
  describe("getBalance", () => {
    it("should return correct balance in SOL", async () => {
      ...
      const publicKeyBuffer = new Uint8Array(
        (await crypto.subtle.exportKey("spki", pubKey)).slice(12),
      );
      const publicKeyB58 = bs58.encode(publicKeyBuffer);

      const sendMock = jest
        .fn()
        .mockResolvedValueOnce({ value: 100_000_000_000_000n });
  ...
  describe("exportKeyPair", () => {
    it("should export a valid base58 key pair", async () => {
      const wallet = await walletManager.generateWallet();
      const [privateKeyBs58, publicKeyBs58] =
        await walletManager.exportKeyPair(wallet);
    ...
    it("should throw on failure", async () => {
      ...
      await expect(walletManager.exportKeyPair(wallet)).rejects.toThrow(
        "Failed to export keypair: crypto failed",
      );
    });
  });
  ...
  describe("importEd25519PrivateKey", () => {
    it("should import a valid 32-byte raw private key", async () => {
      ...
      const cryptoKey =
        await walletManager.importEd25519PrivateKey(rawPrivateKey);
    ...
    it("should throw an error if the key is not 32 bytes", async () => {
      ...
      await expect(
        walletManager.importEd25519PrivateKey(shortKey),
      ).rejects.toThrow("Ed25519 private key must be 32 bytes.");
    });
  });
  ...
  describe("importEd25519PublicKey", () => {
    it("should import a valid 32-byte raw public key", async () => {
      ...
      const cryptoKey =
        await walletManager.importEd25519PublicKey(rawPublicKey);
    ...
    it("should throw an error if the key is not 32 bytes", async () => {
      ...
      await expect(
        walletManager.importEd25519PublicKey(longKey),
