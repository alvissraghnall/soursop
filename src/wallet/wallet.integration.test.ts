...
describe("WalletManager Integration Tests", () => {
  ...
  const VALID_MNEMONIC_PUBLIC_KEY =
  ...
  describe("Mnemonic Import", () => {
    ...
    test("should reject an empty mnemonic", async () => {
      await expect(walletManager.importFromMnemonic("")).rejects.toThrow(
        "Failed to import from mnemonic",
      );
    });
  });
  ...
  describe("Private Key Import", () => {
    test("should import wallet from a Base58 private key", async () => {
      ...
      const [privKeyBs58, pubKeyBs58] =
        await walletManager.exportKeyPair(originalWallet);

      const importedWallet =
        await walletManager.importFromPrivateKey(privKeyBs58);
      const [, importedPubKey] =
        await walletManager.exportKeyPair(importedWallet);
  ...
  describe("Balance Operations", () => {
    ...
    test("should throw an error when checking balance for an invalid address", async () => {
      await expect(
        walletManager.getBalance("invalid-solana-address"),
