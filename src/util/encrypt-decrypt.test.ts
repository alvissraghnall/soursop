  ...
  describe("encrypt function", () => {
    ...
    it("should handle long strings", async () => {
      ...
      expect(result.length).toBeGreaterThanOrEqual(
        SALT_LEN + IV_LEN + AUTH_TAG_LEN + longText.length,
      );
    });
    ...
    it("should reject with scrypt errors", async () => {
      ...
      crypto.scrypt = jest
        .fn()
        .mockImplementation((password, salt, keylen, callback) => {
          callback(mockError);
        });

      await expect(encrypt(testPlaintext, testPassword)).rejects.toThrow(
        "Scrypt error",
      );
