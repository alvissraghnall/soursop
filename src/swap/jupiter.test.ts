...
const mockedConvertJupiterInstructionToKit =
  convertJupiterInstructionToKit as jest.Mock;
const mockedCreateTransactionMessage = createTransactionMessage as jest.Mock;
const mockedSetTransactionMessageFeePayerSigner =
  setTransactionMessageFeePayerSigner as jest.Mock;
const mockedSetTransactionMessageLifetimeUsingBlockhash =
  setTransactionMessageLifetimeUsingBlockhash as jest.Mock;
const mockedAppendTransactionMessageInstructions =
  appendTransactionMessageInstructions as jest.Mock;
const mockedCompileTransaction = compileTransaction as jest.Mock;
const mockedSignTransactionMessageWithSigners =
  signTransactionMessageWithSigners as jest.Mock;
const mockedGetSignatureFromTransaction =
  getSignatureFromTransaction as jest.Mock;
...
describe("jupiter", () => {
  ...
  beforeEach(() => {
    ...
    mockedSetTransactionMessageFeePayerSigner.mockImplementation((_, tx) => ({
      ...tx,
      feePayer: "set",
    }));
    mockedSetTransactionMessageLifetimeUsingBlockhash.mockImplementation(
      (_, tx) => ({ ...tx, lifetime: "set" }),
    );
    const finalMockTxMessage = { message: "final" };
    mockedAppendTransactionMessageInstructions.mockReturnValue(
      finalMockTxMessage,
    );
  ...
  describe("getJupiterQuote", () => {
    ...
    it("should correctly calculate amount in smallest unit", async () => {
      ...
      expect(capturedUrl.searchParams.get("amount")).toBe(
    ...
    it("should throw an error if the fetch request fails", async () => {
      ...
      ).rejects.toThrow(`Failed to get quote: ${errorMessage}`);
    });
  ...
  describe("getSwapInstructions", () => {
    ...
    it("should throw a FetchError if the response is not ok", async () => {
      ...
      await expect(
        getSwapInstructions(mockQuoteResponse, mockUserPublicKey),
      ).rejects.toThrow(
  ...
  describe("executeSwap", () => {
    it("should successfully execute a swap with all instructions", async () => {
      ...
      expect(mockedCreateTransactionMessage).toHaveBeenCalledWith({
        version: 0,
      });
      expect(mockedSetTransactionMessageFeePayerSigner).toHaveBeenCalledWith(
        mockSigner,
        expect.any(Object),
      );
      expect(
        mockedSetTransactionMessageLifetimeUsingBlockhash,
      ).toHaveBeenCalledWith(mockLatestBlockhash, expect.any(Object));

      const expectedInstructions = Array(5).fill(mockKitInstruction);
      expect(mockedAppendTransactionMessageInstructions).toHaveBeenCalledWith(
        expectedInstructions,
        expect.any(Object),
      );

      const signedTx =
        await mockedSignTransactionMessageWithSigners.mock.results[0].value;
      expect(mockClient.sendAndConfirmTransaction).toHaveBeenCalledWith(
        signedTx,
    ...
    it("should handle missing optional instructions", async () => {
      ...
      expect(mockedConvertJupiterInstructionToKit).toHaveBeenCalledWith(
        mockJupiterInstruction,
      );
    ...
    it("should propagate errors from sendAndConfirmTransaction", async () => {
      ...
      await expect(
        executeSwap(mockWallet, mockSwapInstructions),
